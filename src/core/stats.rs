...
mod tests {
    use super::*;
    use crate::core::model::{Gender, Student};
    use std::collections::HashMap;

    #[test]
    fn test_calculate_statistics() {
        let mut classes = vec![Class::new(0), Class::new(1)];

        classes[0].add_student(create_student("S1", 600.0));
        classes[0].add_student(create_student("S2", 700.0));
        classes[1].add_student(create_student("S3", 650.0));
        classes[1].add_student(create_student("S4", 650.0));
    ...
    fn test_gender_balance() {
        ...
        assert!(balance.male_variance >= 0.0);
        assert!(balance.female_variance >= 0.0);
    }
